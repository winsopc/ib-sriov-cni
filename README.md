...
* `publishPfGuid` (boolean, optional): When the allocated device is a PF in VFIO mode, publish the `mellanox.infiniband.connection-details` pod annotation — a JSON map of IB device index to PF node GUID — so consumers (e.g. [ib-kubernetes](https://www.github.com/Mellanox/ib-kubernetes)) can bind only the PFs actually allocated to a pod in multi-VM-per-node deployments. Defaults to false. Requires the CNI to reach the Kubernetes API to patch the pod (see `kubeconfig`).
* `kubeconfig` (string, optional): Path to a kubeconfig the CNI uses to `get`/`patch` the pod when `publishPfGuid` is enabled. Generate one per node from the DaemonSet ServiceAccount with `thin_entrypoint --gen-kubeconfig`; that account needs RBAC to `get` and `patch` pods in the target namespaces. The reference DaemonSet grants this cluster-wide (a `ClusterRoleBinding`) because tenant pods may land in arbitrary namespaces; to confine the feature to known namespaces, bind the same `ClusterRole` with per-namespace `RoleBinding`s instead.
* `logFile` (string, optional): Path for node-local trace lines of the connection-details publisher (pod, interface, PCI device, IB index, GUID), useful for validating the pre-gate publish path since CNI stderr is otherwise buried in kubelet/Multus retry logs.
