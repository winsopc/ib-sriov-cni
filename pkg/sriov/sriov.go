...
func (s *sriovManager) handleVFGuidConfiguration(conf *types.NetConf, pfLink netlink.Link) error {
	if conf.GUID != "" {
		return s.configureVFGuid(conf, pfLink)
	}
	return s.validateVFGuid(conf)
}

// configureVFGuid configures the VF GUID, saving the current GUID for later restoration
func (s *sriovManager) configureVFGuid(conf *types.NetConf, pfLink netlink.Link) error {
